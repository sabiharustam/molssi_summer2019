...
def calculate_tail_corrections(box_length, cutoff, number_particles):
...
def calculate_total_pair_energy(coordinates, box_length, cutoff2):
	# You only use this function at the very beginning and use only once.
	e_total = 0.0
	particle_count = len(coordinates)
...
def adjust_displacement(n_trial, n_accept, max_displacement):
	acc_rate = float(n_accept) / float(n_trial)
	if acc_rate < 0.38:
	...
	return max_displacement, n_trial, n_accept
...
tail_correction = calculate_tail_corrections(box_length, simulation_cutoff, num_particles)
n_trial = 0
n_accept = 0
...
for i_step in range(n_steps):
	...
	if tune_displacement:
		max_displacement, n_trials, n_accept = adjust_displacement(n_trial, n_accept, max_displacement)

print(calculate_total_pair_energy(coordinates, 10, 9.0))
